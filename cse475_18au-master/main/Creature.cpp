...
void Creature::_updateDistance(uint8_t addr, int8_t rssi) {
  // TODO: implement
  uint8_t decay = GLOBALS.STARTLE_DECAY;
  int8_t distance = (decay - rssi) / decay;
...
bool Creature::_rxStart(uint8_t len, uint8_t* payload) {
  ...
  // 0x00XX start in XX state; 0x0000 start in random state; 0x01xx continue
  if (mode == 0x01){
    // continue
  } else if (mode == 0x00) {
    // start in certain state
    if (stateId != 0xFF && stateId > 0x06) {
      //invalid state
      Serial.println("The stateId specified is invalid(0x00 to 0x06 only).");
      Serial.print("Current stateId value: ");
      Serial.println(stateId);
      return false;
    } else {
      // go to the valid state
      uint8_t stateId_next;
      if (stateId == 0x00) {
        // random start
        stateId_next = (uint8_t) rand()%7;
        if (stateId_next == 0) {
          // random stateId == 0, go to ambient 1???
          stateId_next = ++stateId_next;
        }
      } else {
        stateId_next = stateId;
      }
      // change to specified stateId, either from random or stateId
      if (stateId_next == 0xFF) {
        setNextState(new Startle(*this));
      } else {
        if (stateId_next % 2 == 0) {
          setNextState(new Ambient(*this, stateId_next));
        } else {
          setNextState(new Active(*this, stateId_next));
        }
      }
    }
  } else {
    // invalid mode
    Serial.println("The mode specified is invalid(0x00, 0x01 only).");
...
bool Creature::_rxBroadcastStates(uint8_t len, uint8_t* payload) {
  if(GLOBALS.NUM_CREATURES + 1 != len) 
    return false;
